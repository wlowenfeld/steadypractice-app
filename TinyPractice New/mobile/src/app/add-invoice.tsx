...
export default function AddInvoiceScreen() {
  ...
  const handleSave = () => {
    ...
    if (existingInvoice) {
      // Unlink sessions that were previously on this invoice but have been removed
      const previouslyLinked = allSessions.filter(
        (s) => s.invoiceId === existingInvoice.id && !selectedSessionIds.includes(s.id)
      );
      previouslyLinked.forEach((s) => updateSession(s.id, { invoiceId: undefined }));

      updateInvoice(existingInvoice.id, invoiceData);
      // Link sessions to this invoice
      if (selectedSessionIds.length > 0) {
        const linked = linkSessionsToInvoice(selectedSessionIds, existingInvoice.id);
        if (!linked) {
          Alert.alert(
            'Linking Failed',
            'Could not link sessions to invoice. Please link them manually from the invoice screen.'
          );
        }
      }
      router.back();
      ...
      if (selectedSessionIds.length > 0) {
        const linked = linkSessionsToInvoice(selectedSessionIds, newInvoiceId);
        if (!linked) {
          Alert.alert(
            'Linking Failed',
            'Could not link sessions to invoice. Please link them manually from the invoice screen.'
