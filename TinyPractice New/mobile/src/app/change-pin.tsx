...
export default function ChangePinScreen() {
  ...
  const { changePin, pinLockedUntil } = useSecurity();
  ...
  const handleSave = async () => {
    ...
    if (isLockedOut) {
      setError(`Too many attempts. Try again in ${Math.floor(lockoutRemaining / 60)}:${String(lockoutRemaining % 60).padStart(2, '0')}`);
      return;
    }
