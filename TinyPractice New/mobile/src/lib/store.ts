...
interface AppState {
  ...
  linkSessionsToInvoice: (sessionIds: string[], invoiceId: string) => boolean;
    ...
    (set, get) => ({
      ...
      deleteSession: (id) => {
        ...
        set((state) => ({
          ...
          // Remove this session from every invoice's sessionIds array (defensive/bi-directional)
          invoices: state.invoices.map((inv) => ({
            ...inv,
            sessionIds: inv.sessionIds.filter((sid) => sid !== id),
          })),
        }));
      },
      ...
      getAppointmentsForDate: (date) => {
        // Compare using local date strings to avoid timezone conversion issues
        // Both 'date' param and stored scheduledDate are ISO strings
        const targetDateStr = date.split('T')[0];
        return get()
          .appointments.filter((a) => {
            const appointmentDateStr = a.scheduledDate.split('T')[0];
