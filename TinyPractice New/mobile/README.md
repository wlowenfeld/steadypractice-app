...
### API Key Security
- **Secret API keys (OpenAI, Anthropic, Grok, ElevenLabs, Google) are stored server-side only** in the backend `.env` files — never in the mobile app bundle
- **Only public client SDK keys** (RevenueCat, backend URL) are included in `eas.json` and the mobile `.env`
- RevenueCat keys are designed to be public (client-side SDK) and are safe in the app bundle
- If AI API integrations are added in the future, all calls must go through the backend proxy — never call AI APIs directly from the mobile app
